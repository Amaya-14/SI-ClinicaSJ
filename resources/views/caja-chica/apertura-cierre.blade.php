...
@section('content_header')
    <div class="card">
        <div class="card-header">
            <h3 class="card-title m-0">Lista de aperturas y cierres</h3>
        ...
        <div class="card-body">
            <div class="d-flex justify-content-end align-items-center mb-3">
                <x-adminlte-button class="btn-sm bg-teal" label="Nuevo registro" icon="fas fa-plus" data-toggle="modal"
                    data-target="#createAperturaCierre" />
            </div>
            <div>
                @livewire('data-table.tabla-aperturas-cierres')
            </div>
        </div>
        <!-- card-body -->
...
@section('content')
    ...
    @livewire('modal.create.modal-apertura-cierre')
    @livewire('modal.update.modal-apertura-cierre')
@stop
    ...
    <script>
        ...
        modal.forEach(element => {
            element.addEventListener('hidden.bs.modal', function(event) {
                if (this.id == 'updateAperturaCierre' && btnEditar[0].classList.contains('d-none')) {
                    inputs[0].setAttribute('disabled');
                    btnEditar[0].classList.remove('d-none');
                    btnHidden[0].classList.add('d-none');
                    btnHidden[1].classList.add('d-none');
